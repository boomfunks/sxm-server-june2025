...
# New function to modify responses like the Requestly extension
def modify_response(response_json):
    if isinstance(response_json, dict):
        if 'result' in response_json:
            response_json['result']['success'] = True
            response_json['result']['status'] = 'success'
            if 'errors' in response_json['result']:
                response_json['result']['errors'] = []
        
        if 'result' in response_json and 'result' in response_json['result']:
            inner_result = response_json['result']['result']
            inner_result['status'] = "success"
            inner_result['message'] = ""
            inner_result['validation'] = True
            inner_result['showExpiredPopUp'] = False
            inner_result['showIneligiblePopUp'] = False
            inner_result['showRIDIneligiblePopUp'] = False
            inner_result['showOverrideErrorPopUp'] = False
            inner_result['showRedeemedPopUp'] = False
            inner_result['showInvalidPopUp'] = False
...
def appconfig():
    ...
    return modify_response(response.json())
...
def login():
    ...
    return modify_response(response.json()).get('claims_token').get('value')
